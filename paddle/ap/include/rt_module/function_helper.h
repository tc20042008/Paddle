...
struct FunctionHelper {
  adt::Result<axpr::Value> Apply(const Function& function,
    ...
    ADT_LET_CONST_REF(default_ret_val, GetDefaultVal(func_declare->ret_type));
    ArgValue ret_val{default_ret_val};
    ...
    for (int i = 0; i < args.size(); ++i) {
      ...
      ADT_LET_CONST_REF(arg_value, CastToArgValue<axpr::Value>(arg));
      ...
      ADT_LET_CONST_REF(ptr, GetAddrAsVoidPtr(arg_value));
      void_ptr_args.emplace_back(ptr);
    }
    ADT_RETURN_IF_ERR(function->dl_function.Apply(ret, void_ptr_args.data()));
    return ret_val.template CastTo<axpr::Value>();
  }

  adt::Result<ArgValue> GetDefaultVal(const ArgType& arg_type) {
    return arg_type.Match(
        [&](const axpr::DataType& data_type) -> adt::Result<ArgValue> {
          ADT_LET_CONST_REF(data_value, GetDataTypeDefaultVal(data_type));
          return data_value;
        },
        [&](const axpr::PointerType& pointer_type) -> adt::Result<ArgValue> {
  ...
  adt::Result<void*> GetAddrAsVoidPtr(const ArgValue& arg_value) {
    return arg_value.Match(
        [&](const axpr::DataValue& data) -> void* {
          return data.Match(
              [&](const adt::Undefined&) -> void* {
                static_assert(
              ...
              [&](const auto& impl) -> void* {
                using T = std::decay_t<decltype(impl)>;
                return const_cast<T*>(&impl);
              });
        },
        [&](const axpr::PointerValue& ptr) -> void* {
          return ptr.Match([&](const auto& impl) -> void* {
            using T = std::decay_t<decltype(impl)>;
            return const_cast<T*>(&impl);
          });
        });
