...
struct PirProgramMethodClass {
  ...
  adt::Result<adt::List<axpr::Value>> ConvertToOps(
    ...
    for (const auto* op : ops) {
      ADT_LET_CONST_REF(op_data, ConverToOpData(op_index++, op, value2index));
      ret->emplace_back(op_data);
    }
    return ret;
  }

  adt::Result<axpr::Value> ConverToOpData(
      int64_t op_index,
  ...
  adt::Result<adt::List<axpr::Value>> ConvertToValues(
    ...
    for (pir::Value value : values) {
      ADT_LET_CONST_REF(value_data,
                        ConverToValueData(value_index++, value, op2index));
      ret->emplace_back(value_data);
    }
    return ret;
  }

  adt::Result<axpr::Value> ConverToValueData(
      int64_t value_index,
