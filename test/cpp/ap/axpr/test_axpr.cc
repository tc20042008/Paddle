...
TEST(CoreExpr, operator_eq_0) {
  ...
  CoreExpr lhs = core.ComposedCallAtomic(
      core.Lambda({Var{"c"}},
                  core.ComposedCallAtomic(core.Var(kBuiltinIdentity()),
      ...
      core.Var("op2"),
      {});
  CoreExpr rhs = core.ComposedCallAtomic(core.Var(kBuiltinIdentity()),
                                         core.Var(kBuiltinList()),
                                         {
                                             core.Var("a"),
                                             core.Var("a"),
...
TEST(CoreExpr, ConvertCoreExprToAnfExpr) {
  CoreExprBuilder core{};
  using Var = tVar<std::string>;
  CoreExpr core_expr = core.ComposedCallAtomic(
      ...
      core.Var("op0"),
      {});
  AnfExpr anf_expr = ConvertCoreExprToAnfExpr(core_expr);
  auto anf = AnfExprBuilder();
  AnfExpr expected = anf.Let(
      {
          anf.Bind("a", anf.Call(anf.Var("op0"), {})),
          anf.Bind("b", anf.Call(anf.Var("op1"), {})),
          anf.Bind("c", anf.Call(anf.Var("op2"), {})),
